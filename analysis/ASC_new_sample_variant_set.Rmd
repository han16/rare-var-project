...
for ( i in 1:length(top_10gene))
{
  ...
  top_10gene_new_sample[i,1]=sum(ASC_new_sample %>% filter(isIndel=="FALSE" & QD >= 10 & AS_SOR <= 3 & AS_ReadPosRankSum >= -0.8) %>%  filter(Gnomad_non_neuro_AF <0.05 & Gene==top_10gene[i]) %>% select(Transmitted_proband) %>% pull)
  top_10gene_new_sample[i,2]=sum(ASC_new_sample %>% filter(isIndel=="FALSE" & QD >= 10 & AS_SOR <= 3 & AS_ReadPosRankSum >= -0.8) %>%filter(Gnomad_non_neuro_AF<0.05 & Gene==top_10gene[i]) %>% select(Untransmitted_proband) %>% pull)
}
