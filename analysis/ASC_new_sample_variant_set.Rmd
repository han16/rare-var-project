...
old_data_liftover=Anno.Data_liftover %>% select(seqnames, start, ExacAF)
...
old_pos_liftover=unlist(gregexpr("1", old_data_liftover$seqnames)) # locate the position of "1"
old_ID_liftover=paste(substr(old_data_liftover$seqnames, 4, old_pos_liftover), old_data_liftover$start, sep=":")
...
old_liftover_new_combine=old_data1_liftover %>% full_join(new_data1) # combine two data sets: old_liftover & new
  
old_new_combine1=old_new_combine %>% drop_na();
old_liftover_new_combine1=old_liftover_new_combine %>% drop_na()
...
g3=ggplot(old_liftover_new_combine1, aes(x=AF_new, y=AF_old_liftover))+geom_point()+
  geom_abline(intercept =0, slope = 1, color="red")+
  #xlab(expression(paste("Burden: -log" [10], "(p value)")))+
  #ylab(expression(paste("MIRAGE_VS: -log" [10], "(p value)")))+
   ggtitle("AF: new vs old sample after liftover")+
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5, size=15))  # center the title
...
* left panel: 344 (0.0188%) overlapping loci with numeric AF from both old and new sample; one example is `7:100683050 0.042 (AF_old)      0.0000816(AF_new)`, in Gnomad database, V2 say AF is `4.05e-2`, ExAC AF is `4.22e-2`, and v3 (hg38) AF is `3.28e-5`. This example suggests `Gnomad_non_neuro_AF` is close to genomewide AF under hg38. 
...
* in the two figures in the bottom, old samples are liftover, a larger number of loci (45,886, 2.57%) are overlapping between new sample and liftover sample of old data.  
...
* for locus `15:22927986`, `Gnomad_non_neuro_AF=0`, but query from [Gnomad database](https://gnomad.broadinstitute.org/region/15-22927986-22927986?dataset=gnomad_r3) indicates of `1.31e-2` under version `gnomAD v3.1.2 (hg38)`, but no allele frequency under version `gnomAD v2.1.1 (hg19)` 
