...
```{r, echo=F, message=F, warning=F}
rare_syn_variants_new_sample=ASC_new_sample %>% filter(Consequence=="synonymous_variant" ) %>% select(Variant, Gene, Gnomad_non_neuro_AF,  Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)
...
Trans_variant=rare_syn_variants_new_sample %>%filter(Gnomad_non_neuro_AF>0.05 )%>%select(Transmitted_proband)%>%pull
Untrans_variant=rare_syn_variants_new_sample%>% filter(Gnomad_non_neuro_AF>0.05)%>%select(Untransmitted_proband)%>%pull
...
######## category 3 all syn
Trans_variant=rare_syn_variants_new_sample%>%select(Transmitted_proband)%>%pull
Untrans_variant=rare_syn_variants_new_sample %>%select(Untransmitted_proband)%>%pull
fisher_test=fisher.test(matrix(c(sum(Trans_variant), num_proband, sum(Untrans_variant), num_proband), nrow=2))
...
syn_varint_burden=tibble(OR=c(OR[3], OR[1:2]), var=c( "(1): all", "(2): AF>5%", "(3): AF<5%"), lower=c(CI_lower[3], CI_lower[1:2]), upper=c(CI_upper[3], CI_upper[1:2]))
...
######## category 2 1%<AF<5%
Trans_variant=rare_syn_variants_new_sample %>% filter(Gnomad_non_neuro_AF>0.01 &Gnomad_non_neuro_AF<0.05 )%>%select(Transmitted_proband)%>%pull
Untrans_variant=rare_syn_variants_new_sample %>% filter(Gnomad_non_neuro_AF>0.01 & Gnomad_non_neuro_AF<0.05)%>%select(Untransmitted_proband)%>%pull
fisher_test=fisher.test(matrix(c(sum(Trans_variant), num_proband, sum(Untrans_variant), num_proband), nrow=2))
OR[2]=fisher_test$estimate; pvalue[2]=fisher_test$p.value; CI_lower[2]=fisher_test$conf.int[1]; CI_upper[2]=fisher_test$conf.int[2]
...
Untrans_variant=rare_syn_variants_new_sample %>% filter(Gnomad_non_neuro_AF>0.05)%>%select(Untransmitted_proband)%>%pull
fisher_test=fisher.test(matrix(c(sum(Trans_variant), num_proband, sum(Untrans_variant), num_proband), nrow=2))
OR[5]=fisher_test$estimate; pvalue[5]=fisher_test$p.value; CI_lower[5]=fisher_test$conf.int[1]; CI_upper[5]=fisher_test$conf.int[2]


syn_varint_burden=tibble(OR=c(OR[5], OR[2:4], OR[1]), var=c( "(1): AF>5%", "(2): 1%<AF<5%", "(3): 0.1%<AF<1%", "(4): AF<0.1%", "(5): private"), lower=c(CI_lower[5], CI_lower[2:4], CI_lower[1]), upper=c(CI_upper[5], CI_upper[2:4], CI_upper[1]))
...
{
rare_syn_variants_new_sample=ASC_new_sample %>% filter(Consequence=="synonymous_variant"  & VQSLOD<vqslod.cutoff[i]) %>% select(Variant, Gene, Gnomad_non_neuro_AF, Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)
...
odds.ratio.summary=tibble(vqslod=rep(vqslod.cutoff, 5), OR=c( odds.ratio[,5], odds.ratio[,2], odds.ratio[,3], odds.ratio[,4], odds.ratio[,1]), var=c(rep("(1): AF>5%", num.cutoff), rep("(2): 1%<AF<5%", num.cutoff), rep("(3): 0.1%<AF<1%", num.cutoff), rep("(4): AF<0.1%", num.cutoff), rep("(5): private", num.cutoff)))
...
#### filter `VQSLOD` in different gene sets 
