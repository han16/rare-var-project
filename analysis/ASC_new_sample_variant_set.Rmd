...
ASC_new_sample=ASC_new_sample2%>%mutate(Transmitted_proband=Transmitted_proband$Transmitted_proband, Untransmitted_proband=Untransmitted_proband$Untransmitted_proband, Transmitted_proband_parents=Transmitted_proband_parents, Untransmitted_proband_parents=Untransmitted_proband_parents)
...
#### Private variants stratified by AF 
...
```{r, echo=F}
rare_syn_variants_new_sample_private=ASC_new_sample %>% filter(Consequence=="synonymous_variant" & Gnomad_non_neuro_AF<0.05 & (Transmitted_proband_parents+Untransmitted_proband_parents)==1) %>% select(Variant, Gene, Gnomad_non_neuro_AF, Transmitted_proband, Untransmitted_proband)
OR=numeric(); pvalue=numeric(); CI_lower=numeric(); CI_upper=numeric()

######## category 1 
Trans_variant=rare_syn_variants_new_sample_private %>% filter(Gnomad_non_neuro_AF>0.01)%>%select(Transmitted_proband)%>%pull
Untrans_variant=rare_syn_variants_new_sample_private %>% filter(Gnomad_non_neuro_AF>0.01)%>%select(Untransmitted_proband)%>%pull
...
######## category 2 
Trans_variant=rare_syn_variants_new_sample_private %>% filter(Gnomad_non_neuro_AF>0.005 & Gnomad_non_neuro_AF<0.01)%>%select(Transmitted_proband)%>%pull
Untrans_variant=rare_syn_variants_new_sample_private %>% filter(Gnomad_non_neuro_AF>0.005 & Gnomad_non_neuro_AF<0.01)%>%select(Untransmitted_proband)%>%pull
fisher_test=fisher.test(matrix(c(sum(Trans_variant), num_proband, sum(Untrans_variant), num_proband), nrow=2))
...
######## category 3 
Trans_variant=rare_syn_variants_new_sample_private %>% filter(Gnomad_non_neuro_AF>0.001 & Gnomad_non_neuro_AF<0.005)%>%select(Transmitted_proband)%>%pull
Untrans_variant=rare_syn_variants_new_sample_private %>% filter(Gnomad_non_neuro_AF>0.001 & Gnomad_non_neuro_AF<0.005)%>%select(Untransmitted_proband)%>%pull
fisher_test=fisher.test(matrix(c(sum(Trans_variant), num_proband, sum(Untrans_variant), num_proband), nrow=2))
...
######## category 4 
Trans_variant=rare_syn_variants_new_sample_private %>% filter(Gnomad_non_neuro_AF<0.005)%>%select(Transmitted_proband)%>%pull
Untrans_variant=rare_syn_variants_new_sample_private %>% filter(Gnomad_non_neuro_AF<0.005)%>%select(Untransmitted_proband)%>%pull
fisher_test=fisher.test(matrix(c(sum(Trans_variant), num_proband, sum(Untrans_variant), num_proband), nrow=2))
...
syn_varint_private_burden=tibble(OR=OR, var=c("(1): 1%<AF<5%", "(2): 0.5%<AF<1%", "(3): 0.1%<AF<0.5%", "(4): AF<0.1%"), lower=CI_lower, upper=CI_upper)

ggplot(syn_varint_private_burden, aes(x=var, y=OR, fill=var)) + # draw the OR plot 
  geom_bar(stat="identity", color="black", 
           position=position_dodge()) +
  ggtitle("Burden of private variants stratified by AF")+
  geom_errorbar(aes(ymin=lower, ymax=upper), width=.2,
                 position=position_dodge(.9))+
