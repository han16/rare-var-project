...
ASC_new_sample_liftover2=ASC_new_sample_liftover1 %>% filter(!str_detect(Consequence, 'intron_variant'))%>%filter(isIndel=="FALSE" & QD >= 10 & AS_SOR <= 3 & AS_ReadPosRankSum >= -0.8) %>% filter(isSyn=="FALSE") %>% filter(Gnomad_non_neuro_AF<0.05)# exclude indel & apply other filters for SNP's
```
...
ASC_new_sample=ASC_new_sample%>% 
  filter(isIndel=="FALSE" & QD >= 1 & AS_SOR <= 3 & AS_ReadPosRankSum >= -0.8) %>% filter(!str_detect(Consequence, 'intron_variant')) # exclude indel & apply other filters for SNP's
...
* `QD>=10.5  & AS_SOR <=3 & AS_ReadPosRankSum>-0.8 & isIndel=="FALSE"`

```{r, echo=F}
CYFIP1=ASC_new_sample%>%filter(Gene=="CYFIP1" & isSyn=="FALSE" & Gnomad_non_neuro_AF<0.05) %>% filter(QD>=10  & AS_SOR <=3 & AS_ReadPosRankSum>-0.8 & isIndel=="FALSE")  %>% filter(!str_detect(Consequence, 'intron_variant')) %>% select(Variant, Chrom, Gene, Consequence,Gnomad_non_neuro_AF, Transmitted_proband, Untransmitted_proband  )
...
### comparison between old and new sample after liftover under hg19 
