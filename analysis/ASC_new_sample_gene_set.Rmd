...
#    cat(i, "th gene of ", "\t", num.gene, "\t", "is running", "\n")
    ...
    if (length(var.index.list)>0) # calculate Bayes factor for variant (i,j)
      ...
      {
        if (new.data$group.index[var.index.list[j]]<=6)   # note here bar.gamma is group specific, attention what bar.gamma goes to which group 
...
```{r, echo=F, message=F, warning=F, eval=F}
BF_para_all_est=as_tibble(read.csv("../output/kyleData/BF.proband.AllGene_1015_2022.csv"))
...
fisher.test(matrix(c(overlap1, overlap2, 10, length(all.gene)), nrow=2))
