...
## Burden of syn var with PLI cutoff 
...
```{r, echo=F, message=F, warning=F}
syn_variants_new_sample=ASC_new_sample %>% filter(isSyn!="FALSE" & as.numeric(as.character(pLI))>=0.9 & as.numeric(as.character(Gnomad_non_neuro_AF))<0.001 & as.numeric(as.character(VQSLOD)) >= 5.13) %>% select(Variant, Gene, Gnomad_non_neuro_AF,  Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)
