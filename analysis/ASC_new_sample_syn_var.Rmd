...
```{r burdern analysis-gene level, results='hide', echo=F, warning=F, message=F, eval=F}
# read into gene set 
source("C:\\Shengtong\\Research\\rare-var\\rare-var-project\\analysis\\Read.into.Gene.set.R")
```
...
###########################################
ASC_new_sample3=as_tibble(read.table("C:\\Shengtong\\Research\\rare-var\\ASC\\ASC_v17_raw_TDT_by_parent_2021-05-10.txt", sep='\t',  header=T, fill=T))
#  argument "sep='\t' " matters !!!!!!!!!!!!!!!! this is the correct way to read the data by Kyle's code 
```
...
```{r, echo=F}
############## this is using the sample ASC_new_sample3, the data read in the correct way  
...
##### add new columns of total transmitted and non-transmitted variants  
ASC_new_sample_correct=ASC_new_sample3%>%mutate(Transmitted_proband=Transmitted_proband$Transmitted_proband, Untransmitted_proband=Untransmitted_proband$Untransmitted_proband, Transmitted_proband_parents=Transmitted_proband_parents$Transmitted_proband_parents, Untransmitted_proband_parents=Untransmitted_proband_parents$Untransmitted_proband_parents)
...
* use data read in the correct way
* filter ` isSyn==TRUE`
* AF from `Gnomad_non_neuro_AF` is used to define other categories. 
* When use AF cutoff, be aware of 15614 variants with zero AF going to which category 

```{r, echo=F, message=F, warning=F}
syn_variants_new_sample_correct=ASC_new_sample_correct %>% filter(isSyn=="TRUE" ) %>% select(Variant, Gene, Gnomad_non_neuro_AF,  Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)

AF_cutoff=c(0, 0.05, 1) # define AF cutoffs 
OR=numeric(); pvalue=numeric(); CI_lower=numeric(); CI_upper=numeric()
for (i in 1:(length(AF_cutoff)-1))
{
  burden_test=fisher_test(syn_variants_new_sample_correct, num_proband, AF_cutoff[i], AF_cutoff[i+1])
  OR[i]=burden_test$OR; pvalue[i]=burden_test$pvalue; CI_lower[i]=burden_test$CI_lower; CI_upper[i]=burden_test$CI_upper
}
burden_test=fisher_test(syn_variants_new_sample_correct, num_proband, AF_cutoff[1], AF_cutoff[3])
  OR[i+1]=burden_test$OR; pvalue[i+1]=burden_test$pvalue; CI_lower[i+1]=burden_test$CI_lower; CI_upper[i+1]=burden_test$CI_upper
# 3 categories; (1) AF<5% (2) AF>5% (3) all 
...
#*  filter `Consequence=="synonymous_variant"`
#
#* Private variants are defined as variants with total #allele  count equal to  1 in parents calculated as #`Transmitted_proband_parents+Untransmitted_proband_paren#ts=1`
#
#* AF from `Gnomad_non_neuro_AF` is used to define other #categories. 
...
burden_test=fisher_test(syn_variants_new_sample_correct %>% filter((Transmitted_proband_parents+Untransmitted_proband_parents)==1), num_proband, AF_cutoff[1], AF_cutoff[5]) # private var  
  OR[i+1]=burden_test$OR; pvalue[i+1]=burden_test$pvalue; CI_lower[i+1]=burden_test$CI_lower; CI_upper[i+1]=burden_test$CI_upper
# 5 categories; (1) private (2) AF<0.1% (3) 0.1%=<AF<1% (4) 1%=<AF<5% (5) 5%=<AF
...
```
> summary(ASC_new_sample_correct %>% filter(isSyn=="TRUE") %>% select(VQSLOD)%>%pull())
     Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
-39450.00      6.02      6.88      4.02      7.70     11.24
```

```
Kyle looked at using a VQSLOD cutoff of 3.75, but then brought it up to 5.13 after trying a similar calibration procedure with non-coding indels.
```
* data read in the correct way 
...
* blue vertical line is VQSLOD=5.13 and red line is 9. 

```{r, echo=F, message=F, warning=F}
num.cutoff=100
AF_cutoff=c(0, 0.001, 0.01, 0.05, 1)
#summary(ASC_new_sample %>% filter(Consequence=="synonymous_variant" & Gnomad_non_neuro_AF<0.05)%>%select(VQSLOD)%>%pull)
vqslod.cutoff=seq(-40,11,length.out=num.cutoff)
odds.ratio=matrix(nrow=num.cutoff, ncol=5)
for (i in 1:num.cutoff)
{
syn_variants_new_sample_correct=ASC_new_sample_correct %>% filter(isSyn=="TRUE"  & VQSLOD<vqslod.cutoff[i]) %>% select(Variant, Gene, Gnomad_non_neuro_AF, Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)

for (j in 1:(length(AF_cutoff)-1)) # at every VQSLOD cutoff, consider 5 variant categories 
{
  burden_test=fisher_test(syn_variants_new_sample_correct, num_proband, AF_cutoff[j], AF_cutoff[j+1])
  odds.ratio[i,j]=burden_test$OR;
  #pvalue[i]=burden_test$pvalue; #CI_lower[i]=burden_test$CI_lower; CI_upper[i]=burden_test$CI_upper
}  # end of for (j in 1:(length(AF_cutoff)-1))
burden_test=fisher_test(syn_variants_new_sample_correct %>% filter((Transmitted_proband_parents+Untransmitted_proband_parents)==1), num_proband, AF_cutoff[1], AF_cutoff[5]) # private var  
...
ggplot(odds.ratio.summary, aes(x=vqslod, y=OR, group=var)) +
  geom_line(aes(color=var))+
  ylim(c(0,1))+
  geom_vline(xintercept = 5.13, linetype="solid", 
                color = "blue", size=1.5)+
...
for (i in 1:num.cutoff)
{
syn_variants_new_sample_correct=ASC_new_sample_correct %>% filter(isSyn=="TRUE"  & QD >= 3 & AS_SOR <= 3  & AS_ReadPosRankSum>ReadPosRankSum.cutoff[i]) %>% select(Variant, Gene, Gnomad_non_neuro_AF, Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)
...
for (i in 1:num.cutoff)
{
syn_variants_new_sample_correct=ASC_new_sample_correct %>% filter(isSyn=="TRUE"  & QD >= 1 & AS_SOR <= 3  & AS_ReadPosRankSum>ReadPosRankSum.cutoff[i]) %>% select(Variant, Gene, Gnomad_non_neuro_AF, Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)
...
for (i in 1:num.cutoff)
{
syn_variants_new_sample_correct=ASC_new_sample_correct %>% filter(isSyn=="TRUE"  & QD >= QD.cutoff[i] & AS_SOR <= 3  & AS_ReadPosRankSum>-0.8) %>% select(Variant, Gene, Gnomad_non_neuro_AF, Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)
...
for (i in 1:num.cutoff)
{
syn_variants_new_sample_correct=ASC_new_sample_correct %>% filter(isSyn=="TRUE"  & AS_SOR <= SOR.cutoff[i] & QD>=1  & AS_ReadPosRankSum>-0.8) %>% select(Variant, Gene, Gnomad_non_neuro_AF, Transmitted_proband, Untransmitted_proband, Transmitted_proband_parents, Untransmitted_proband_parents)
