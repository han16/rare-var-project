...
save_folder="C:/Shengtong/Research/rare-var/RareVariant/CodeandDataforPaper/Data/"
...
for (i in seq_along(rep_use_index)) {
  ...
  simulated_data[[i]] <- list(     # output simulated_data for reproducibility 
  Ui                     = Ui,
  mirage_pvalue          = mirage_pvalue,
  mirage_BF              = mirage_BF,
  fisher_pvalue          = fisher_pvalue,
  skato_pvalue           = skato_pvalue,
  cmc_pvalue             = cmc_pvalue,
  asum_pvalue            = asum_pvalue,
  acat.pvalue            = acat.pvalue,
  fisher.separate.pvalue = fisher.separate.pvalue
)
...
names(simulated_data)=paste0("delta_", delta)

#saveRDS(simulated_data, file =paste0(save_folder, "simulated_data_random_num_variants.rds"))
...
```{r, message=F, warning=F, eval=F}
all <- grid.arrange(p, nrow=1) 
ggsave(all,filename = paste0("C:/Shengtong/Research/rare-var/RareVariant/AJHG submisson/round1 revision/AdditionalSimulations/RandomNumberVariants/num_of_genes_identified.pdf"),width = 5.5,height = 3,dpi = 600)
```
...
```{r, message=F, warning=F}
library(dplyr)
tp_mirage=df_combined %>% filter(Method=="MIRAGE") %>% select(TP_Value) %>% pull
tp_skato=df_combined %>% filter(Method=="SKAT-O") %>% select(TP_Value) %>% pull

paste0("power improves:", round((tp_mirage-tp_skato)/tp_skato*100,4),  "%")
