...
ggplot(result.summary, aes(x=Features, y=OR, fill = Features))+
  ...
  geom_point(aes(x=Features, y=-log(pval, base=10),color=Features, size=-log(pval, base=10)),
...
#  cat(i, "th subcom is running", "\n")
  ...
  if (nrow(order.overlap.data)==0)  
   comb.summ.MIRAGE[(i-1)*length(gene.set)+j,((k-1)*4+1):(k*4)]=c(pois.test$odds.ratio, NA, pois.test$rate.case, pois.test$rate.contr) 
...
save(comb.summ, comb.summ.MIRAGE, file="../output/Burden_MIRAGE_Comparison.RData")
```
...
load("../output/Burden_MIRAGE_Comparison.RData")
...
ggplot(burden_mirage_pvalue, aes(x=cate, y=-log(pvalue, base=10), fill=method))+
  geom_bar(stat="identity", width=.5, position = "dodge")+xlab("Features")+ylab("-log(pvalue, base=10)")+
