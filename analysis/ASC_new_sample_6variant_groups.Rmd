...
for (i in 1:length(interest_gene))
{
 # i=1143
  ...
  burden_test=fisher.test(matrix(c(LoF_burden_005[i,1],num.family, LoF_burden_005[i,2], num.family), nrow=2))
  LoF_burden_005[i,3]=burden_test$estimate
  LoF_burden_005[i,4]=burden_test$p.value
  p_value_005[i,1]=LoF_burden_005[i,4]
  
  ########## missense variants 
  Missense_burden_005[i,1]=sum(single_gene_data %>% filter(Gnomad_non_neuro_AF<0.05 & Group_Index>3) %>% select(Transmitted_proband)%>% pull)  
  Missense_burden_005[i,2]=sum(single_gene_data %>% filter(Gnomad_non_neuro_AF<0.05 & Group_Index>3) %>% select(Untransmitted_proband)%>% pull)
  ...
  Missense_burden_005[i,4]=burden_test$p.value
  ...
  LoF_Missense_burden_005[i,1]=sum(single_gene_data %>% filter(Gnomad_non_neuro_AF<0.05) %>% select(Transmitted_proband)%>% pull)  
  LoF_Missense_burden_005[i,2]=sum(single_gene_data %>% filter(Gnomad_non_neuro_AF<0.05) %>% select(Untransmitted_proband)%>% pull)
  ...
  LoF_burden_001[i,4]=burden_test$p.value
  p_value_001[i,1]=LoF_burden_001[i,4]
  
  ########## missense variants 
  Missense_burden_001[i,1]=sum(single_gene_data %>% filter(Gnomad_non_neuro_AF<0.01 & Group_Index>3) %>% select(Transmitted_proband)%>% pull)  
  Missense_burden_001[i,2]=sum(single_gene_data %>% filter(Gnomad_non_neuro_AF<0.01 & Group_Index>3) %>% select(Untransmitted_proband)%>% pull)
  ...
  Missense_burden_001[i,4]=burden_test$p.value
   ############## avoid p value of 0 or 1 
  ...
  ########## LoF+missense variants 
  LoF_Missense_burden_001[i,1]=sum(single_gene_data %>% filter(Gnomad_non_neuro_AF<0.01) %>% select(Transmitted_proband)%>% pull)  
  LoF_Missense_burden_001[i,2]=sum(single_gene_data %>% filter(Gnomad_non_neuro_AF<0.01) %>% select(Untransmitted_proband)%>% pull)
  ...
  LoF_Missense_burden_001[i,4]=burden_test$p.value
...
################
  ...
  colnames(p_value_005)=c("LoF_pvalue", "LoF+Missense_pvalue", "fisher_combine_pvalue")
 ...
 # write.csv(p_value_005, file="../output/KyleData/BF.proband.AllGene_6vargroups_burden_pvalues_AF0.05_1123_2022.csv")
#  write.csv(combine_005, file="../output/KyleData/BF.proband.AllGene_6vargroups_burden_counts_AF0.05_1123_2022.csv")

################
  rownames(p_value_001)=interest_gene
  colnames(p_value_001)=c("LoF_pvalue", "LoF+Missense_pvalue", "fisher_combine_pvalue")
  
  combine_001=cbind(LoF_burden_001, Missense_burden_001, LoF_Missense_burden_001)
  rownames(combine_001)=interest_gene
  colnames(combine_001)=paste(rep(c("LoF", "Missense", "LoF+Missense"), each=4), c("T", "UT", "OR", "P_value"), sep="_")
#  write.csv(p_value_001, file="../output/KyleData/BF.proband.AllGene_6vargroups_burden_pvalues_AF0.01_1123_2022.csv")
#  write.csv(combine_001, file="../output/KyleData/BF.proband.AllGene_6vargroups_burden_counts_AF0.01_1123_2022.csv")
...
* extremely small p values are from variants with unbalancedly more variants in controls than in cases. 
