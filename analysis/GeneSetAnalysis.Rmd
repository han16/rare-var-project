...
```{r, echo=F}
BF.const.gene.common.delta=read.csv("D:\\ResearchWork\\StatisticalGenetics\\Rare-variant-project\\rare-var-project\\output\\BF.constraint.gene.ASD.commmondelta0.1.csv", header=T)
...
```{r, echo=F}
kable(rank.BF.const.gene.common.delta[1:20,1:2], row.names=NA, caption =("Top 20 genes of constraint genes with fixed delta"), "html")%>%
kable_styling() %>%
scroll_box(height = "200px")
```
...
```{r, echo=F}
BF.const.gene.specific.delta=read.csv("D:\\ResearchWork\\StatisticalGenetics\\Rare-variant-project\\rare-var-project\\output\\BF.constraint.gene.ASD.specificdelta.csv", header=T)
...
para.est.sum=matrix(nrow=2, ncol=8)
para.est.sum[1,]=para.est
para.est.sum[2,]=para.summ[,1]
colnames(para.est.sum)=seq(1,8)
barplot(para.est.sum, beside=T, col=c("darkblue", "red"), legend=c(expression(paste("common ", delta, "=0.1")), expression(paste("gene specific ", delta[i]))), ylim=c(0,1), ylab=expression(paste(hat(beta))), cex.lab=0.7) 
dev.off()
```
