...
para.est$cate.index=para.est$cate.index %>% recode("1"="LoF; 1%<AF<5%", "2"="LoF; AF<1%", "3" = "Damaging; 1%<AF<5%","4"= "Damaging; 0.1%<AF<1%", "5" ="Damaging; AF<0.1%", "6" ="Non-damaging; 1%<AF<5%", "7" ="Non-damaging; 0.1%<AF<1%", "8" ="Non-damaging; AF<0.1%")
...
```{r, echo=F}
#BF_para_all_est=loadRData("C:\\Shengtong\\Research\\rare-var\\rare-var-project\\output\\para.est.constraint.gene.estimate.delta.RData")
#BF_para_all_est=loadRData("..\\output\\para.est.constraint.gene.estimate.delta.RData")
BF_para_all_est=load("..\\output\\para.est.constraint.gene.estimate.delta.RData")
# Bayes_factor=as_tibble(BF_para_all_est) old code doesn't work???
Bayes_factor=as_tibble(BF.gene)
...
strong ASD gene score 1,2,3 : enrichment is 8/10 vs 76/1003 gives OR 10.5058 and p value $1.98\times 10^{-5}$

strong ASD gene score 1,2: enrichment is 4/10 vs 31/1003 gives OR 12.8417 and p value $8.24\times 10^{-4}$
...
```{r, echo=F}
# BF_para_all_est=loadRData("../output/para.est.constraint.gene.estimate.delta.RData") # old code that doesn't work 
BF_para_all_est=load("../output/para.est.constraint.gene.estimate.delta.RData")
...
#top.const.overlap.withdenovo=length(intersect(top1000.denovo.gene, BF_para_all_est$Gene)) # old code 
top.const.overlap.withdenovo=length(intersect(top1000.denovo.gene, BF.gene$Gene))
...
######## Top 10 mirage genes 
top10mirage.enrichment.tidy$group =factor(top10mirage.enrichment.tidy$group, levels=c("Strong.ASD", "Plausible.ASD", "DNM")) # re order lables in x axis 

pvalues=c(0.000825, 0.0028, 0.00025)  # use fisher exact test in R fisher.test to get p values 
#### for example, fisher.exact.test(matrix(c(4, round(0.0309*1003), 10,  1003), nrow=2))=0.000825

#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/top_mirage_gene_enrichment.pdf")
top10mirage_enrichment=ggplot(top10mirage.enrichment.tidy, aes(group, percent)) + 
  geom_bar(aes(fill = class), stat = "identity", position = "dodge")+
  theme_classic()+
  xlab("")+ylab("Proportion of putative ASD genes")+
  ylim(c(0, 0.8))+
  theme(plot.title = element_text(hjust = 0.5, size=20, face="bold"))+
  ggtitle("Enrichment of ASD genes in MIRAGE results")+
  #theme(plot.title = element_text(size=12))
   theme(legend.title = element_blank())+
  geom_signif(annotations ="p=8.3E-4", y_position = 0.45  ,xmin=0.7, xmax=1.2, size=2, vjust=-0.5, textsize=10)+
  geom_signif(annotations ="p=2.8E-3", y_position = 0.45  ,xmin=1.7, xmax=2.2, size=2, vjust=-0.5, textsize=10)+
  geom_signif(annotations ="p=2.5E-4", y_position = 0.75  ,xmin=2.7, xmax=3.2, size=2, vjust=-0.5, textsize=10)+
...
const.gene.pvalue=loadRData("../output/const.gene.pvalue.other.methods.include.CALPHA.RData")
...
const.gene.pvalue.adj=const.gene.pvalue%>%mutate("SKATO.pvalue.adj"=p.adjust(SKATO.pvalue, method="BH"))%>%
  ...
  mutate("CALPHA.pvalue.adj"=p.adjust(CALPHA.pvalue, method="BH")) 
...
const.gene.BF.pvalue.combine %>% filter(post_prob>0.52)%>%select(Gene, BF, post_prob, CALPHA.pvalue.adj) %>% arrange(desc(post_prob)) # FDR<0.3


const.gene.BF.pvalue.combine %>% filter(post_prob>0.4)%>%select(Gene, BF, post_prob) %>% arrange(desc(post_prob))

CALPHA_gene=const.gene.BF.pvalue.combine %>% filter(CALPHA.pvalue.adj<0.2) %>% select(Gene,BF, post_prob, CALPHA.pvalue.adj)
CALPHA_gene %>% arrange(CALPHA.pvalue.adj) %>% print(n=7)

right_join(as_tibble(const.gene.BF), CALPHA_gene, by="Gene")%>% select(-post_prob.y, -BF.y)%>%arrange(desc(post_prob.x)) 
left_join(as_tibble(MIRAGE_gene), CALPHA_gene, by="Gene")%>% select(-post_prob.y, -BF.y)%>%arrange(desc(post_prob.x))%>% print(n=29) ## ASUM: does not work for genes with one variant, resutling in NA 
```
...
```{r, echo=F}
####### enirchment of top 10 calpha genes 
CALPHA_gene_sort=CALPHA_gene %>% arrange(CALPHA.pvalue.adj)
top10_CALPHA_gene=CALPHA_gene_sort$Gene[1:10]


top10CALPHA.percent=c(length(intersect(top10_CALPHA_gene, strong.ASD.gene)), length(intersect(top10_CALPHA_gene, plausible.ASD.gene)), length(intersect(top10_CALPHA_gene, top1000.denovo.gene)))/10
const.gene.percent=c(length(intersect(all.const.gene, strong.ASD.gene)), length(intersect(all.const.gene, plausible.ASD.gene)), length(intersect(all.const.gene, top1000.denovo.gene)))/1003
top10CALPHA.enrichment=tibble(class=c("Top10 CALPHA Gene", "All 1003 Constraint Gene"), Strong.ASD=c(top10CALPHA.percent[1], const.gene.percent[1]), Plausible.ASD=c(top10CALPHA.percent[2], const.gene.percent[2]), DNM=c(top10CALPHA.percent[3], const.gene.percent[3]))
top10CALPHA.enrichment.tidy=top10CALPHA.enrichment %>% gather( group, percent,2:4)

pvalue=numeric()
for (i in 1:3)
pvalue[i]=fisher.test(matrix(c(top10CALPHA.percent[i]*10, const.gene.percent[i]*1003, 10, 1003 ), nrow=2))$p.value

#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/top_CALPHA_gene_enrichment.pdf")
top10CALPHA_enrichment=ggplot(top10CALPHA.enrichment.tidy, aes(group, percent)) + 
  geom_bar(aes(fill = class), stat = "identity", position = "dodge")+
  theme_classic()+
  xlab("")+ylab("Proportion of putative ASD genes")+
  ylim(c(0, 0.8))+
  theme(plot.title = element_text(hjust = 0.5, size=15, face="bold"))+
  #ggtitle("Enrichment of ASD genes in CALPHA results")+
  #theme(plot.title = element_text(size=12))
   theme(legend.title = element_blank())+
  geom_signif(annotations ="p=0.29", y_position = 0.15  ,xmin=0.7, xmax=1.2, size=1, vjust=-0.5, textsize=5)+
  geom_signif(annotations ="p=1.00", y_position = 0.15  ,xmin=1.7, xmax=2.2, size=1, vjust=-0.5, textsize=5)+
  geom_signif(annotations ="p=0.60", y_position = 0.15  ,xmin=2.7, xmax=3.2, size=1, vjust=-0.5, textsize=5)+
  #geom_signif(annotations =0.000825, y_position = 0.45  ,xmin=0.7, xmax=1.2)+
   theme(legend.position="bottom")+
theme(text = element_text(size=15))+
theme(legend.spacing.x = unit(1.0, 'cm')) # add space between symbols in legend 
top10CALPHA_enrichment
dev.off()
```
...
#write.csv(Gene_CYFIP1_par, file="../output/HighlightGene/Gene_CYFIP1_par.csv")
Gene_CYFIP1_merge=Gene_CYFIP1_par %>% gather(status, No.var, 3:4) 
Gene_CYFIP1_merge$variant.group=Gene_CYFIP1_merge$group.index %>% recode("1"="LoF; 1%<AF<5%", "2"="LoF; AF<1%", "3" = "Damaging; 1%<AF<5%","4"= "Damaging; 0.1%<AF<1%", "5" ="Damaging; AF<0.1%", "6" ="Non-damaging; 1%<AF<5%", "7" ="Non-damaging; 0.1%<AF<1%", "8" ="Non-damaging; AF<0.1%")
...
```{r, echo=F}
ggplot(as_tibble(Gene_CYFIP1_merge), aes(x=ID, y=No.var, color=status, shape=variant.group))+
  geom_point()+
  ggtitle("CYFIP1")+
  theme_classic()+
...
```{r, echo=F}
CYFIP1.gene=loadRData("../output/para.est.constraint.gene.estimate.delta.full.info.CYFIP1.gene.RData")
eta.est=c(0, 0.41, 0, 0, 0.16, 0, 0, 0.17)
var.BF=CYFIP1.gene$var.BF
group.index=CYFIP1.gene$group.index

adj.var.BF=numeric()

for (i in 1:length(var.BF))
  adj.var.BF[i]=(1-eta.est[group.index[i]])+eta.est[group.index[i]]*var.BF[i]
prod(adj.var.BF)
top.var.index=which(CYFIP1.gene$No.case==8)
prod(adj.var.BF[-top.var.index])

post.prob=tibble(post.prob=eta.est[group.index]*var.BF/(1-eta.est[group.index]+eta.est[group.index]*var.BF))
ggplot(post.prob, aes(x=post.prob))+
  geom_histogram(position="dodge2", color="blue", fill="blue")+
theme_classic()+
  xlab("Post probability")+
  ylab("")+
  ggtitle("Histogram of posterior probability of individual variants in gene CYFIP1")+
  theme(plot.title = element_text(hjust = 0.5))+  # center the title 
theme(text = element_text(size=10)) # size=20 is good enough
...
#### highlight gene EP400


```{r, echo=F}
EP400.gene=loadRData("../output/para.est.constraint.gene.estimate.delta.full.info.EP400.gene.RData")
eta.est=c(0, 0.41, 0, 0, 0.16, 0, 0, 0.17)
var.BF=EP400.gene$var.BF
group.index=EP400.gene$group.index
...
theme_classic()+
  xlab("Post probability")+
  ylab("")+
  ggtitle("Histogram of posterior probability of individual variants in gene EP400")+
  theme(plot.title = element_text(hjust = 0.5))+  # center the title 
theme(text = element_text(size=10)) # size=20 is good enough
```
...
```{r, echo=F}
Gene_EP400=Anno.Data[which(Anno.Data$Gene=="EP400"),]
Gene_EP400_par=eight.partition(Gene_EP400)
#write.csv(Gene_CYFIP1_par, file="../output/HighlightGene/Gene_CYFIP1_par.csv")
Gene_EP400_merge=Gene_EP400_par %>% gather(status, No.var, 3:4) 
Gene_EP400_merge$variant.group=Gene_EP400_merge$group.index %>% recode("1"="LoF; 1%<AF<5%", "2"="LoF; AF<1%", "3" = "Damaging; 1%<AF<5%","4"= "Damaging; 0.1%<AF<1%", "5" ="Damaging; AF<0.1%", "6" ="Non-damaging; 1%<AF<5%", "7" ="Non-damaging; 0.1%<AF<1%", "8" ="Non-damaging; AF<0.1%")
Gene_EP400_merge$status=Gene_EP400_merge$status %>% recode(No.case="Case", No.contr="Control")
```

```{r, echo=F}
ggplot(as_tibble(Gene_EP400_merge), aes(x=ID, y=No.var, color=status, shape=variant.group))+
  geom_point()+
  ggtitle("EP400")+
  theme_classic()+
  xlab("Coordinate")+ylab("#.var")+
...
sx <- sort(-log(theo, base=10)); sy <- sort(-log(Burden.pvalue, base=10))
lenx <- length(sx)
leny <- length(sy)
...
burden=tibble(sx=sx, sy=sy)
 qqplot_burden=ggplot(burden) + geom_point(aes(x=sx, y=sy))+
   theme_classic()+
...
theo <- (ppoints(length(SKATO.pvalue)))
  ...
  #qqplot(-log(theo, base=10), -log(SKATO.pvalue, base=10), xlab=expression(paste("Theoretical ",-log[10], "(p-values)")), ylab="", main="(B) SKATO", frame=F, cex.lab=0.8, pch=20)
  #abline(0,1,col='red', lwd=2)
#dev.off()    
 

 sx <- sort(-log(theo, base=10)); sy <- sort(-log(SKATO.pvalue, base=10))
lenx <- length(sx)
leny <- length(sy)
if (leny < lenx)sx <- approx(1L:lenx, sx, n = leny)$y
if (leny > lenx)sy <- approx(1L:leny, sy, n = lenx)$y

 skato=tibble(sx=sx, sy=sy)
 qqplot_skato=ggplot(skato) + geom_point(aes(x=sx, y=sy))+
   theme_classic()+
   xlab(expression(paste("Theoretical ",-log[10], "(p-values)")))+
   ylab(expression(paste("Observed ", -log[10], "(p-values)")))+
   #ylab("")+
   theme(plot.title = element_text(hjust = 0.5, size=30,face="bold"))+  #center the title+
   ggtitle("SKAT-O")+
   geom_abline(intercept = 0, slope = 1, color="red", 
                 linetype="solid", size=1.5)+
theme(text = element_text(size=30))
...
ggarrange(qqplot_burden, qqplot_skato, top10mirage_enrichment, nrow=1)
#dev.off()
```
...
burden=tibble(sx=sx, sy=sy)
 qqplot_burden=ggplot(burden) + geom_point(aes(x=sx, y=sy))+
   ...
   xlab(expression(paste("Theoretical ",-log[10], "(p-values)")))+
   ylab(expression(paste("Observed ", -log[10], "(p-values)")))+
   theme(plot.title = element_text(hjust = 0.5, size=12,face="bold"))+  #center the title+
   ggtitle("(A) Burden")+
   geom_abline(intercept = 0, slope = 1, color="red", 
                 linetype="solid", size=1.5)
...
skato=tibble(sx=sx, sy=sy)
 qqplot_skato=ggplot(skato) + geom_point(aes(x=sx, y=sy))+
   ...
   geom_abline(intercept = 0, slope = 1, color="red", 
                 linetype="solid", size=1.5)
#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/Fig-pvalue_QQplot_othermethod_ASD_topconstgene_LoFDamaging.pdf")
ggarrange(qqplot_burden, qqplot_skato, nrow=1)
#dev.off()
################  
