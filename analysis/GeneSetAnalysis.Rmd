...
result.summary=data.frame(Cate=Cate, OR=OR, pval=pval)
# pdf("../../Figure/burden_constgene.pdf")
  ggplot(result.summary, aes(x=Cate, y=OR, fill=Cate))+
  geom_bar(stat="identity")+
  ylab("")+xlab("Variant Categories")+ ggtitle("Burden analysis for eight categories")+
...
```{r, echo=F}
prior_cons=0.18
cons_post=Bayes_factor %>% mutate(post_prob=(1-prior_cons)*BF/(prior_cons+(1-prior_cons)*BF))

tau=seq(0, 0.99, by=0.01)
num_pred=NULL
false_disc=NULL
FDR=NULL
...
tau_fdr=tibble(tau=tau, fdr=FDR)
...
```{r, echo=T}
MIRAGE_gene=const.gene.BF.pvalue.combine %>% filter(post_prob>0.92)%>%select(Gene, BF, post_prob)
MIRAGE_gene%>%arrange(desc(post_prob))%>%print(n=29)
...
method    | FDR=0.05  | FDR=0.1  | FDR=0.2
---------|--------|---------------|-----------
MIRAGE  | 29  | 131 | 749
SKATO   | 0  | 0 | 0
CMC  | 0 | 0 | 0
...
20 gene selected by ASUM are "ADAR"    "CHST1"   "PLXNB1"  "INO80"   "TRIM8"   "TRIM3"   "RBM14"   "ZIC2"    "ZSCAN10" "SMARCA4" "AP2A1"  "DHX30"   "TRA2B"   "NR2F1"   "EBF1"    "NRF1"    "ABCA2"   "TUBG1"   "PCBP1"   "MAFB

29 genes selected by MIRAGE at FDR=0.05 are "UBR4"     "ADAR"     "EP400"    "CHD8"     "METTL3"   "DYNC1H1"  "GLG1"     "ACACA"    "SNRNP200" "ANAPC1"  "PLXNB1"   "KALRN"    "MACF1"    "CYFIP1"   "INO80"    "FBN1"     "SRCAP"    "NOTCH3"   "TP63"     "SLC4A8"   "BAZ1A"    "MYH10"    "TRIP12"   "HIRA"     "TOP3B"    "CACNA1D"  "DOCK4"    "WHSC1L1"  "ABCA2" 




When FDR=0.05, there are 4 genes "ADAR"   "PLXNB1" "INO80"  "ABCA2" overlapping between MIRAGE and ASUM. 


When FDR=0.1 there are 8 genes overlapping. 


When FDR=0.2, all 20 genes from ASUM are included in genes selected by MIRAGE. 
...
barplot(para.est.sum, beside=T, col=c("darkblue", "red", "green"), legend=c(expression(paste("fixed common", delta, "=0.1")), expression(paste("gene specific ", delta[i])), expression(paste(hat(delta), "=0.18"))), ylim=c(0,1), ylab=expression(paste(hat(beta))), cex.lab=0.7) 
#dev.off()
```
