...
legend(2, 0.8, c(paste("beta", seq(1,8), sep="")), col=seq(1,8), lty=rep(1,8))

```

#### Nested gene sets 

Look at top 10%, 20%, 30%, 40%, 50% gene sets. 

```{r, echo=T}
beta=matrix(nrow=5, ncol=8)
rownames(beta)=c("top10%", "top20%", "top30%", "top40%", "top50%")
beta[1,]=c(0, 0.6195,  0, 0.0025,  0.0427, 0,  0,  0.0855)
beta[2,]=c(0, 0.4441,  0, 0,   0.0450, 0, 0,  0.0807)
beta[3,]=c(0,  0.3561, 0,   0,   0.0410,0,  0,  0.0775)
beta[4,]=c(0,  0.3137,  0, 0,   0.0397,0,  0,   0.0715)
beta[5,]=c(0,  0.2975,  0, 0,   0.0369,0,   0,  0.0636)
plot(beta[,1], ylim=c(0,1), type="o", xaxt="n", xlab="", ylab=expression(paste(hat(beta))), main="Parameter estimate of five nested gene sets")
axis(side=1, at=1:5, labels=rownames(beta),cex.axis=0.7)
for (i in 2:8)
 lines(beta[,i], type="o", col=i)
...
beta[2,]=c(0, 0.4477,  0, 0,   0.0498, 0, 0,  0.0724)
beta[3,]=c(0,  0.3595, 0,   0,   0.0468,0,  0.0007,  0.0653)
beta[4,]=c(0,  0.3156,  0, 0,   0.0436,0,  0,   0.0601)
beta[5,]=c(0,  0.2979,  0, 0,   0.0427,0,   0,  0.0499)
plot(beta[,1], ylim=c(0,1), type="o", xaxt="n", xlab="", ylab=expression(paste(hat(beta))), main="Parameter estimate of five nested gene sets")
axis(side=1, at=1:5, labels=rownames(beta),cex.axis=0.7)
for (i in 2:8)
 lines(beta[,i], type="o", col=i)
...
top 20% | 51,230/50,454=1.0154 | 46,680/46,645=1.0008
top 30% | 63,601/63,042=1.0089 | 58,715/58,861=0.9975
top 40% | 75,578/75,082=1.0066 | 69,371/69,466=0.9986
top 50% | 86,633/86,233=1.0046 | 78,920/79,371=0.9943
...
5.308590e-01 0.000000e+00 4.727189e-10 4.135604e-02 0.000000e+00
[6] 1.092592e-39 7.942025e-02

quantile <20: 

3.378893e-01 0.000000e+00 4.740355e-39 5.579214e-02 0.000000e+00
[6] 9.055088e-49 7.053990e-02
