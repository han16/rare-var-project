...
anno1=tibble(ID=paste(annotation1$locus, annotation1$ref, annotation1$alt, sep="_"), Gene=annotation1$genes,  Annotation=annotation1$category, ExacAF=annotation1$exac)

ID=paste("chr", annotation2$V1[-1], ":", sep="")
start_end=paste(annotation2$V2[-1], "_", annotation2$V3[-1], "_", annotation2$V4[-1], "_", annotation2$V5[-1], sep="")
...
counts=read.table(paste(path, "pseudo_cc.frq.counts", sep=""), header=T)
bim=read.table(paste(path, "pseudocc.tucc.bim", sep=""))
end=bim$V4+1
case.control=tibble(ID=paste("chr",bim$V1, ":", bim$V4, "_", end, "_", bim$V5, "_", bim$V6, sep=""), No.case=counts$C1, No.contr=counts$C2)

full.data=right_join(annotation, case.control, by="ID") # only keep locus with observed alleles 
