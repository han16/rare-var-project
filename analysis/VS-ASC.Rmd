...
old_variant_set=Anno.Data%>%filter(Annotation %in% LoF.def==F & ExacAF<0.0001)%>%select(ID, Gene, No.case, No.contr) %>% drop_na() # missense variant with AF<1E-4 in old sample
...
cell_variant_set=cell_trans_data_AF %>% filter(Consequence=="[missense_variant]")%>% filter(ExacAF<0.0001) %>% select(Variant, GENE_NAME, Transmitted_proband, Untransmitted_proband)  # missense variant in cell data with AF<1-10-4
transmitted_cell_sample=numeric()
nontransmitted_cell_sample=numeric()
for ( i in 1:length(Gene_Set_List))
 {
  Feature_Data=cell_variant_set%>%filter(GENE_NAME %in% Gene_Set_List[[i]])%>% drop_na()
  if (nrow(Feature_Data)>0)
  { 
