...
Use same 6 variants groups in cell paper, stratify each variant group with AF cutoff, $AF>10^{-4}, 10^{-4}>AF>10^{-5}, AF<10^{-5}$, with a total of 18 variant categories. 
...
Use same 6 variants groups in cell paper, stratify each variant group with AF cutoff, $AF>10^{-4}, AF< 10^{-4}$, leading to a total of 12 variant groups. 
...
To compare old sample and cell data, look at missense variant (regardless of categories) +AF<1E-4+10 gene set and how they compare with each other. 

Old sample: missense: Non-LoF variants  

cell sample: : [missense_variant]
...
```{r, echo=F, warning=F, message=F}
######### obtain missense variants with AF<1-E4+10 gene set in cell data
############## to save burden and mirage-vs result 
Burden_MIRAGE_VS=matrix(nrow=length(Gene_Set_List), ncol=6)
rownames(Burden_MIRAGE_VS)=Gene_Set
colnames(Burden_MIRAGE_VS)=c("Transmitted", "Untransmitted", "Burden_OR", "Burden_Pval", "MIRAGE-VS_eta", "MIRAGE-VS_Pval")
...
rare_missense_variant_cell=cell_trans_data_AF %>% filter(Consequence=="[missense_variant]")%>% filter(ExacAF<0.0001) %>% select(Variant, GENE_NAME, Transmitted_proband, Untransmitted_proband)  # missense variant in cell data with AF<1-10-4
transmitted_cell_sample=numeric()
nontransmitted_cell_sample=numeric()
for ( i in 1:length(Gene_Set_List))
 {
  Feature_Data=rare_missense_variant_cell%>%filter(GENE_NAME %in% Gene_Set_List[[i]])%>% drop_na()
  if (nrow(Feature_Data)>0)
  { 
...
missense_cell_sample=tibble(Gene=rep(Gene_Set,2), status=rep(c("transmitted", "nontransmitted"), each=10), variant_count=c(transmitted_cell_sample, nontransmitted_cell_sample))

g_cell_sample=ggplot(missense_cell_sample, aes(x = Gene, y = variant_count, fill = status)) +
...
#### Burden vs MIRAGE-VS for old sample and cell sample  


```{r, echo=F, warning=F, message=F}
Burden_MIRAGE_VS=read.csv("../output/OldSample/Variant_Set_missense_10Geneset.csv", header=T)
g1=ggplot(as_tibble(Burden_MIRAGE_VS), aes(x=Burden_OR, y=-log(Burden_Pval, base=10), color=as.factor(Burden_MIRAGE_VS$X), shape=as.factor(Burden_MIRAGE_VS$X)))+
scale_shape_manual(values=1:nlevels(as.factor(Burden_MIRAGE_VS$X)))+
  geom_point()+
  xlim(c(0,2))+
  ylim(c(0,8))+
  #geom_hline(yintercept=-log(0.05/num.test, base=10), linetype="dashed", color = "red")+
  #geom_hline(yintercept=-log(burden.pvalue.fdr.0.05.cutoff, base=10), linetype="solid", color = "blue")+
  xlab("OR")+ylab(expression(paste("-log" [10], "(p value)")))+
  ggtitle("Burden-old sample")+
  #theme_classic()+
   theme(legend.position = "none")+  # no legend 
theme(plot.title = element_text(hjust = 0.5)) +# center the title 
theme(legend.title=element_blank())+
#theme(legend.text=element_text(size=30))+ # text size in legend 
#theme(text = element_text(size=35))+
guides(size=FALSE)+   # suppress size legend 
guides(shape = guide_legend(override.aes = list(size = 5))) # symbol size in legend 

#colnames(Burden_MIRAGE_VS)[5:6]=c("MIRAGE_VS_eta","MIRAGE_VS_Pval")
g2=ggplot(as_tibble(Burden_MIRAGE_VS), aes(x=MIRAGE_VS_eta, y=-log(MIRAGE_VS_Pval, base=10), color=as.factor(row.names(Burden_MIRAGE_VS)), shape=as.factor(row.names(Burden_MIRAGE_VS))))+
scale_shape_manual(values=1:nlevels(as.factor(row.names(Burden_MIRAGE_VS))))+
  geom_point()+
#  xlim(c(0,2))+
  ylim(c(0,8))+
  #geom_hline(yintercept=-log(0.05/num.test, base=10), linetype="dashed", color = "red")+
...
```{r, echo=F, warning=F, message=F}
Burden_MIRAGE_VS=read.csv("../output/CellDataASC/Variant_Set_missense_10Geneset.csv", header=T)
g1=ggplot(as_tibble(Burden_MIRAGE_VS), aes(x=Burden_OR, y=-log(Burden_Pval, base=10), color=as.factor(Burden_MIRAGE_VS$X), shape=as.factor(Burden_MIRAGE_VS$X)))+
scale_shape_manual(values=1:nlevels(as.factor(Burden_MIRAGE_VS$X)))+
  geom_point()+
  xlim(c(0,1.5))+
...
colnames(Burden_MIRAGE_VS)[6:7]=c("MIRAGE_VS_eta","MIRAGE_VS_Pval")
g2=ggplot(as_tibble(Burden_MIRAGE_VS), aes(x=MIRAGE_VS_eta, y=-log(MIRAGE_VS_Pval, base=10), color=as.factor(row.names(Burden_MIRAGE_VS)), shape=as.factor(row.names(Burden_MIRAGE_VS))))+
scale_shape_manual(values=1:nlevels(as.factor(row.names(Burden_MIRAGE_VS))))+
  geom_point()+
#  xlim(c(0,2))+
...
consider missense variants with AF<10-E4 in top 10 genes from old sample and cell sample and see how they compare 

```{r, echo=F}
Top_10_gene=c("CYFIP1", "EP400", "FBN1", "SRCAP", "DYNC1H1", "ABCA2", "DOCK4", "CACNA1D", "CHD8", "TRIP12")
### display  rare missense distribution between two samples for specific gene set 
######################################################
...
variant_gene_cell_sample=rare_missense_variant_cell %>% filter(GENE_NAME %in% Gene_Set_List[[7]])
...
cell sample: LoF: protein truncating variants (PTV) (nonsense, frame-shift, and essential splice site variants) with pLI>=0.5
...
### Run MIRAGE-VS on LoF variants of gene set 

Apply mirage on LoF variants in different gene set from old sample and cell data and see how they differ. 




### LoF variants in different constraint genes 

LoF variants in 50 constraint genes. 
