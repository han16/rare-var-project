...
method=rep(c("MIRAGE-LRT", "Burden", "SKATO"),each=3)
```
...
method=rep(c("MIRAGE", "Burden", "SKATO"))
```
...
###  Mixture of risk genes and non-risk genes $0<\delta<1$

Since the simulated data is a mixture of risk genes and non-risk genes, drawing ROC curve is one way to test power of discriminating risk genes of  different methods. For MIRAGE, Bayes factor of every gene were calculated and used to plot ROC, called MIRAGE-BF. 
...
Parameter settings: N1=N0=3000 $\alpha=\alpha_0=0.1$, $\beta=2000, \beta_0=1000$. 100 genes, each having 1000 variants with $\eta=0.1, 0.2, 0.3$ risk variants, but the actural $\eta$ is varying for each generated data set because of the filtering. $\delta=0.5$, i.e. half genes are risk genes and the rest half are nonrisk genes. $\bar{\gamma}=5$. 


```{r, echo=F, warning=F}
load("../output/Varyeta_MixedGene_OneRiskVarGroup/eta_0.01.RData")
eta_0.01_Ui=Gene.Risk.Status
eta_0.01_all_pi=all.pi
eta_0.01_mirage_pvalue=MIRAGE.pvalue
eta_0.01_mirage_BF=MIRAGE.BF
eta_0.01_fisher_pvalue=Fisher.pvalue
eta_0.01_skato_pvalue=SKATO.pvalue

load("../output/Varyeta_MixedGene_OneRiskVarGroup/eta_0.05.RData")
eta_0.05_Ui=Gene.Risk.Status
eta_0.05_all_pi=all.pi
eta_0.05_mirage_pvalue=MIRAGE.pvalue
eta_0.05_mirage_BF=MIRAGE.BF
eta_0.05_fisher_pvalue=Fisher.pvalue
eta_0.05_skato_pvalue=SKATO.pvalue
...
i=1
Ui=eta_0.01_Ui[[i]]
mirage_pvalue=eta_0.01_mirage_pvalue[[i]]
mirage_BF=eta_0.01_mirage_BF[[i]]
fisher_pvalue=eta_0.01_fisher_pvalue[[i]]
skato_pvalue=eta_0.01_skato_pvalue[[i]]
...
Ui=eta_0.05_Ui[[i]]
mirage_pvalue=eta_0.05_mirage_pvalue[[i]]
mirage_BF=eta_0.05_mirage_BF[[i]]
fisher_pvalue=eta_0.05_fisher_pvalue[[i]]
skato_pvalue=eta_0.05_skato_pvalue[[i]]
method=rep(c("MIRAGE-LRT", "Burden", "SKATO", "MIRAGE-BF"), each=num_run)
#roc_single_run=data.frame(D=Ui, m=c(-log(mirage_pvalue, base=10),-log(fisher_pvalue, base=10), -log(skato_pvalue, base=10)), method=method)
roc_single_run=data.frame(D=Ui, m=c(mirage_pvalue,fisher_pvalue,skato_pvalue, -mirage_BF), method=method)
...
p2=p2.basic+
  ...
  ggtitle(expression(paste(eta, "=0.05")))+
theme(plot.title = element_text(hjust = 0.5, size=10))+  #center the title 
  theme(axis.text.x= element_text(size=4))+  # custmorize the axis tick size 
theme(axis.text.y= element_text(size=4))
