...
* variant level:
    ...
    + $U_i=1$,  gene $i$ is a causal gene, the variant $(i,j)$ whose risk status is denoted by $Z_{ij}$ is generated as $Z_{ij} \sim Ber(1, \pi(\eta))$, $\pi(\eta)$ is a function linking annotations to the probability of being causal. 
          ...
          1.   $Z_{ij}=0$ (which is under null),  given the generated $X_{1ij}+X_{0ij}$, split into case and controls by conditional binomial distribution, with $p=\frac{N1}{N1+N0}$. 
          
          2.  $Z_{ij}=1$ (which is under alternative), AF $q_{ij}^{(1)} \sim Beta(\alpha, \beta)$, $\gamma_{ij} \sim Gamma(\bar{\gamma}*\sigma, \sigma)$,  $X_{1ij}+X_{0ij} \sim Pois(N1+N0, q_{ij}^{(1)})$, split into case and control by conditional binomial distribution with $p=\frac{N1*\gamma_{ij}}{N1*\gamma_{ij}+N0}$. 
...
g1=ggplot(powereta01_gather, aes(x=Sample_Size, y=TPR/100, group=Method))+
  ...
  geom_point(aes(color=Method, shape=Method), size=point.size)+
  xlab("N1=N0")+ 
...
powereta03_gather$Method=powereta03_gather$Method %>% recode(MIRAGE_VS="MIRAGE-VS", SKATO="SKAT-O") # change the method name 
...
theme(text = element_text(size=text.size))+
theme(legend.text=element_text(size=text.size))+  # change legend text size
...
#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Main/Fig2-power_comparison_MIRAGE-vs.pdf")
...
########### sample size 1000
gammabar3=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.3.true5.eta.0.4.sample.size1000.replicate1000.RData")
gammabar4=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.4.true5.eta.0.4.sample.size1000.replicate1000.RData")
gammabar5=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.5.true5.eta.0.4.sample.size1000.replicate1000.RData")
gammabar6=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.6.true5.eta.0.4.sample.size1000.replicate1000.RData")
gammabar3_mirage=as.numeric(as_tibble(gammabar3) %>% filter(MIRAGE.pvalue<sig.cutoff) %>% tally())
gammabar3_burden=as.numeric(as_tibble(gammabar3) %>% filter(Fisher.pvalue<sig.cutoff) %>% tally())
gammabar3_skato=as.numeric(as_tibble(gammabar3) %>% filter(SKATO.pvalue<sig.cutoff) %>% tally())

gammabar4_mirage=as.numeric(as_tibble(gammabar4) %>% filter(MIRAGE.pvalue<sig.cutoff) %>% tally())
gammabar4_burden=as.numeric(as_tibble(gammabar4) %>% filter(Fisher.pvalue<sig.cutoff) %>% tally())
gammabar4_skato=as.numeric(as_tibble(gammabar4) %>% filter(SKATO.pvalue<sig.cutoff) %>% tally())

gammabar5_mirage=as.numeric(as_tibble(gammabar5) %>% filter(MIRAGE.pvalue<sig.cutoff) %>% tally())
gammabar5_burden=as.numeric(as_tibble(gammabar5) %>% filter(Fisher.pvalue<sig.cutoff) %>% tally())
gammabar5_skato=as.numeric(as_tibble(gammabar5) %>% filter(SKATO.pvalue<sig.cutoff) %>% tally())

gammabar6_mirage=as.numeric(as_tibble(gammabar6) %>% filter(MIRAGE.pvalue<sig.cutoff) %>% tally())
gammabar6_burden=as.numeric(as_tibble(gammabar6) %>% filter(Fisher.pvalue<sig.cutoff) %>% tally())
gammabar6_skato=as.numeric(as_tibble(gammabar6) %>% filter(SKATO.pvalue<sig.cutoff) %>% tally())

power=tibble(gamma_bar=3:6, MIRAGE_VS=c(gammabar3_mirage, gammabar4_mirage, gammabar5_mirage, gammabar6_mirage), Burden=c(gammabar3_burden, gammabar4_burden, gammabar5_burden, gammabar6_burden), SKATO=c(gammabar3_skato, gammabar4_skato, gammabar5_skato, gammabar6_skato))
power_gather=power %>% gather(Method,TPR, MIRAGE_VS:SKATO)
power_gather_SS1000=power_gather %>% add_column(SampleSize=1000)

power_gather$Method =power_gather$Method %>% recode(MIRAGE_VS="MIRAGE-VS") 
g3=ggplot(power_gather, aes(x=gamma_bar, y=TPR/1000, group=Method))+
  geom_line(aes(color=Method), size=0.8)+
  ylim(0,1)+
  geom_point(aes(color=Method, shape=Method))+
...
########### sample size 3000
gammabar3=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.3.true5.eta.0.4.sample.size3000.replicate1000.RData")
gammabar4=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.4.true5.eta.0.4.sample.size3000.replicate1000.RData")
gammabar5=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.5.true5.eta.0.4.sample.size3000.replicate1000.RData")
gammabar6=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.6.true5.eta.0.4.sample.size3000.replicate1000.RData")
gammabar3_mirage=as.numeric(as_tibble(gammabar3) %>% filter(MIRAGE.pvalue<sig.cutoff) %>% tally())
gammabar3_burden=as.numeric(as_tibble(gammabar3) %>% filter(Fisher.pvalue<sig.cutoff) %>% tally())
gammabar3_skato=as.numeric(as_tibble(gammabar3) %>% filter(SKATO.pvalue<sig.cutoff) %>% tally())
...
power=tibble(gamma_bar=3:6, MIRAGE_VS=c(gammabar3_mirage, gammabar4_mirage, gammabar5_mirage, gammabar6_mirage), Burden=c(gammabar3_burden, gammabar4_burden, gammabar5_burden, gammabar6_burden), SKATO=c(gammabar3_skato, gammabar4_skato, gammabar5_skato, gammabar6_skato))
power_gather=power %>% gather(Method,TPR, MIRAGE_VS:SKATO)
power_gather_SS3000=power_gather %>% add_column(SampleSize=3000)

power_gather$Method =power_gather$Method %>% recode(MIRAGE_VS="MIRAGE-VS") 
g2=ggplot(power_gather, aes(x=gamma_bar, y=TPR/1000, group=Method))+
  geom_line(aes(color=Method), size=0.8)+
  ylim(0,1)+
  geom_point(aes(color=Method, shape=Method))+
  xlab(expression(paste(bar(gamma))))+ 
...
########### sample size 5000
gammabar3=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.3.true5.eta.0.4.sample.size5000.replicate1000.RData")
gammabar4=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.4.true5.eta.0.4.sample.size5000.replicate1000.RData")
gammabar5=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.5.true5.eta.0.4.sample.size5000.replicate1000.RData")
gammabar6=loadRData("../output/Power_Analysis_MIRAGE-VS/varyGammamean/gamma.bar.6.true5.eta.0.4.sample.size5000.replicate1000.RData")
gammabar3_mirage=as.numeric(as_tibble(gammabar3) %>% filter(MIRAGE.pvalue<sig.cutoff) %>% tally())
gammabar3_burden=as.numeric(as_tibble(gammabar3) %>% filter(Fisher.pvalue<sig.cutoff) %>% tally())
gammabar3_skato=as.numeric(as_tibble(gammabar3) %>% filter(SKATO.pvalue<sig.cutoff) %>% tally())
...
power=tibble(gamma_bar=3:6, MIRAGE_VS=c(gammabar3_mirage, gammabar4_mirage, gammabar5_mirage, gammabar6_mirage), Burden=c(gammabar3_burden, gammabar4_burden, gammabar5_burden, gammabar6_burden), SKATO=c(gammabar3_skato, gammabar4_skato, gammabar5_skato, gammabar6_skato))
power_gather=power %>% gather(Method,TPR, MIRAGE_VS:SKATO)
power_gather_SS5000=power_gather %>% add_column(SampleSize=5000)

power_gather$Method =power_gather$Method %>% recode(MIRAGE_VS="MIRAGE-VS")
g1=ggplot(power_gather, aes(x=gamma_bar, y=TPR/1000, group=Method))+
  geom_line(aes(color=Method), size=0.8)+
  ylim(0,1)+
  geom_point(aes(color=Method, shape=Method))+
  xlab(expression(paste(bar(gamma))))+ 
...
power_gather_combine=bind_rows(power_gather_SS1000, power_gather_SS3000, power_gather_SS5000)

#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/Fig1-power_comparison_MIRAGE-vs_varyGammabar.pdf")
power_gather_combine$Method=power_gather_combine$Method %>% recode(MIRAGE_VS="MIRAGE-VS", SKATO="SKAT-O")
ggplot(power_gather_combine, aes(x=gamma_bar, y=TPR/1000, group=Method))+
  geom_line(aes(color=Method), size=0.8)+
  ylim(0,1)+
  geom_point(aes(color=Method, shape=Method))+
  xlab(expression(paste(bar(gamma))))+ 
...
To investigate the type one error, we simulated 1000 independent data sets, with varying sample size $N1=N0=1000, 3000, 5000$, 100 variants in one group; no risk variants, i.e., $\eta=0$ and other parameters are pretty the same as before 
$\alpha_0=0.1, \beta_0=1000,\alpha= 0.1, \beta=2000,\bar{\gamma}=5,\sigma= 1$. The basic idea is to see the proportion of significant test under null. 


```{r, echo=F}
null.pvalue.SS1000=loadRData("../output/TypeOneError/Simulation_OneGene_OneNonrisk_VarGroup_SS1000_replicate1000.RData")
null.pvalue.SS1000.gather=null.pvalue.SS1000 %>% gather(Method, pvalue, 2:4)
null.pvalue.SS1000.combine=null.pvalue.SS1000.gather %>% add_column(SampleSize=1000)

null.pvalue.SS3000=loadRData("../output/TypeOneError/Simulation_OneGene_OneNonrisk_VarGroup_SS3000_replicate1000.RData")
null.pvalue.SS3000.gather=null.pvalue.SS3000 %>% gather(Method, pvalue, 2:4)
null.pvalue.SS3000.combine=null.pvalue.SS3000.gather %>% add_column(SampleSize=3000)

null.pvalue.SS5000=loadRData("../output/TypeOneError/Simulation_OneGene_OneNonrisk_VarGroup_SS5000_replicate1000.RData")
null.pvalue.SS5000.gather=null.pvalue.SS5000 %>% gather(Method, pvalue, 2:4)
null.pvalue.SS5000.combine=null.pvalue.SS5000.gather %>% add_column(SampleSize=5000)

null.pvalue.combine=bind_rows(null.pvalue.SS1000.combine, null.pvalue.SS3000.combine, null.pvalue.SS5000.combine)

null.pvalue.combine$Method=null.pvalue.combine$Method %>% recode(MIRAGE.pvalue="MIRAGE-VS", Fisher.pvalue="Burden", SKATO.pvalue="SKATO")

###### loot at count with p value less than 0.05 
null.pvalue.combine.significant=null.pvalue.combine %>% filter(pvalue<0.05)
null.pvalue.combine.significant.count=tibble(Method=c(rep(c("MIRAGE-VS", "Burden", "SKAT-O"),3)), count=c(21,26,31, 14,38,45, 15,37,42),sample.size=rep(c(1000,3000, 5000), each=3))
null.pvalue.combine.significant.count$sample.size=as.factor(null.pvalue.combine.significant.count$sample.size)
#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/Fig2-typeIerror_MIRAGE-vs.pdf")
ggplot(null.pvalue.combine.significant.count, aes(x=sample.size, y=count/1000, fill=Method)) +
  geom_bar(stat="identity", position="dodge")+
  geom_hline(yintercept=0.05, linetype="dashed", color = "red")+
  xlab("N1=N0")+ylab("False positive (p value < 5%) rate" )+
  theme_classic()
#dev.off()
#ggplot(null.pvalue.SS5000.gather, aes(x=pvalue, fill=Method))+
#geom_histogram(position="dodge2")+
#  ylim(c(0,100))+
#  xlab("p value")  

####### this is for drawing histogram ########## 
#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/Fig2-typeIerror_MIRAGE-vs.pdf")
#ggplot(null.pvalue.combine, aes(x=pvalue, fill=Method))+
#  geom_histogram(position="dodge2")+
#  theme(axis.text = element_text(size = 10))+ # customize the font size of axis label
  #ylim(c(0,100))+
...
```{r, echo=F}
null.pvalue.truegamma5.use3=loadRData("../output/TypeOneError/Simulation_OneGene_OneNonrisk_VarGroup_SS3000_truegamma5_use3_replicate1000.RData")
null.pvalue.truegamma5.use3.gather=null.pvalue.truegamma5.use3 %>% gather(Method, pvalue, 2:4)
null.pvalue.truegamma5.use3.combine=null.pvalue.truegamma5.use3.gather %>% add_column(gammabar=3)

null.pvalue.truegamma5.use4=loadRData("../output/TypeOneError/Simulation_OneGene_OneNonrisk_VarGroup_SS3000_truegamma5_use4_replicate1000.RData")
null.pvalue.truegamma5.use4.gather=null.pvalue.truegamma5.use4 %>% gather(Method, pvalue, 2:4)
null.pvalue.truegamma5.use4.combine=null.pvalue.truegamma5.use4.gather %>% add_column(gammabar=4)

null.pvalue.truegamma5.use5=loadRData("../output/TypeOneError/Simulation_OneGene_OneNonrisk_VarGroup_SS3000_truegamma5_use5_replicate1000.RData")
null.pvalue.truegamma5.use5.gather=null.pvalue.truegamma5.use5 %>% gather(Method, pvalue, 2:4)
null.pvalue.truegamma5.use5.combine=null.pvalue.truegamma5.use5.gather %>% add_column(gammabar=5)

null.pvalue.truegamma5.use6=loadRData("../output/TypeOneError/Simulation_OneGene_OneNonrisk_VarGroup_SS3000_truegamma5_use6_replicate1000.RData")
null.pvalue.truegamma5.use6.gather=null.pvalue.truegamma5.use6 %>% gather(Method, pvalue, 2:4)
null.pvalue.truegamma5.use6.combine=null.pvalue.truegamma5.use6.gather %>% add_column(gammabar=6)


null.pvalue.combine=bind_rows(null.pvalue.truegamma5.use3.combine, null.pvalue.truegamma5.use4.combine, null.pvalue.truegamma5.use5.combine, null.pvalue.truegamma5.use6.combine)

null.pvalue.combine$Method=null.pvalue.combine$Method %>% recode(MIRAGE.pvalue="MIRAGE-VS", Fisher.pvalue="Burden", SKATO.pvalue="SKATO")

###### loot at count with p value less than 0.05 and manually enter the result
null.pvalue.combine.significant=null.pvalue.combine %>% filter(pvalue<0.05)

null.pvalue.combine.significant.count=tibble(Method=c(rep(c("MIRAGE-VS", "Burden", "SKAT-O"),4)), count=c(22,32,39, 22,46,55, 25,32,44, 11,35,47),gammabar=rep(3:6, each=3))
null.pvalue.combine.significant.count$gammabar=as.factor(null.pvalue.combine.significant.count$gammabar)

#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/Fig2-typeIerror_MIRAGE-vs_misspecified_gammabar.pdf")
ggplot(null.pvalue.combine.significant.count, aes(x=gammabar, y=count/1000, fill=Method)) +
  geom_bar(stat="identity", position="dodge")+
  ggtitle(expression(paste("True ", bar(gamma), "=5")))+
  theme_classic()+
  ...
  geom_hline(yintercept=0.05, linetype="dashed", color = "red")+
  xlab(expression(paste(bar(gamma))))+
ylab("False positive (p value < 5%) rate" )
#dev.off()
```
...
############ sample size 1000
bargamma_2=load("../output/Varybargamma/gammabar_2_samplesize1000.RData")
all.eta[,1]=all.pi[,1]
bargamma_3=load("../output/Varybargamma/gammabar_3_samplesize1000.RData")
all.eta[,2]=all.pi[,1]
bargamma_4=load("../output/Varybargamma/gammabar_4_samplesize1000.RData")
all.eta[,3]=all.pi[,1]
bargamma_5=load("../output/Varybargamma/gammabar_5_samplesize1000.RData")
all.eta[,4]=all.pi[,1]
...
bar.gamma=2:5
eta.summary=data.frame(mean=eta.mean, sd=eta.sd, bar.gamma=bar.gamma)
p2=ggplot(eta.summary, aes(x=bar.gamma, y=mean, group=1)) + 
   geom_point() +
  geom_pointrange(aes(ymin=mean-sd, ymax=mean+sd))+
  ggtitle("N1=N0=3000")+
  theme_classic()+
...
############ sample size 5000
bargamma_2=load("../output/Varybargamma/gammabar_2_samplesize5000.RData")
all.eta[,1]=all.pi[,1]
bargamma_3=load("../output/Varybargamma/gammabar_3_samplesize5000.RData")
all.eta[,2]=all.pi[,1]
bargamma_4=load("../output/Varybargamma/gammabar_4_samplesize5000.RData")
all.eta[,3]=all.pi[,1]
bargamma_5=load("../output/Varybargamma/gammabar_5_samplesize5000.RData")
all.eta[,4]=all.pi[,1]
#bargamma_6=load("../output/Varybargamma/bargamma_6.RData")
#all.eta[,5]=all.pi[,1]
colnames(all.eta)=2:(2+num.gamma-1)
...
bar.gamma=2:5
eta.summary=data.frame(mean=eta.mean, sd=eta.sd, bar.gamma=bar.gamma)
p3=ggplot(eta.summary, aes(x=bar.gamma, y=mean, group=1)) + 
   geom_point() +
  geom_pointrange(aes(ymin=mean-sd, ymax=mean+sd))+
  ggtitle(" N1=N0=5000")+
  theme_classic()+
...
#png("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/Fig3-parameter_estimate.png")
...
p2=ggplot(eta.summary, aes(x=sample.size, y=mean, group=1)) + 
   geom_point() +
  geom_pointrange(aes(ymin=mean-sd, ymax=mean+sd))+
  ggtitle(expression(paste("(II) ", bar(gamma), "=5")))+
  theme(plot.title = element_text(hjust = 0.5, size=10))+  # center the title 
...
#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/Fig3-parameter_estimate.pdf")
...
Parameter settings: N1=N0=3000 $\alpha=\alpha_0=0.1$, $\beta=2000, \beta_0=1000$, $\delta=0.1, 0.2, 0.3, 0.4$. 1000 genes, each having 100 variants in three different groups, the first group has 60% variants, the second has 30% and the rest 10% are in the third group. In each variant group of every gene, the proportion  of risk variants  $\eta$ is varying at 0.05, 0.2, 0.5 and every risk category has its own    $\bar{\gamma}$ respectively. The basic idea is that the rarer the variant is, the more effect size it tends to have. 
...
for (i in 1:nrow(fisher.separate.pvalue))
{
  ...
  # fisher.adj.pvalue[i]=min(fisher.separate.pvalue[i,])
   ...
   ############ this way of calculating ACAT pvalue is not right according to ACAT paper #####
   if (max(fisher.separate.pvalue[i,])==1)
    acat.pvalue[i]=1-1/3  # use calibrated p value https://github.com/yaowuliu/ACAT 
  if (max(fisher.separate.pvalue[i,])<1)
    acat.pvalue[i]=ACAT(fisher.separate.pvalue[i,]) # use ACAT to combine p value from 3 var groups
...
method=rep(c( "Burden(0.556)", "SKAT-O(0.724)", "MIRAGE(0.839)", "CMC(0.755)", "ASUM(0.672)", "ACAT(0.535)"), each=num_run)
...
method=rep(c( "Burden(0.556)", "Burden-adj(0.565)", "Burden-combine(0.437)"), each=num_run)
...
* CMC: use CMC function in R package AssotesteR, with 100 permutations, and `maf=10^(-4)*c(0.05, 0.2, 0.5)`. 

* ACAT p value: within each of 3 variant groups, run ACAT on p value of every single variant by burden test (collapse variant if its MAC<10, as ACAT paper suggested) to obtain group level p value, then use ACAT to combine group level p value to get gene level p values  


```{r,echo=F, message=F, warning=F}
rm("Ui", "mirage_pvalue", "mirage_BF", "fisher_pvalue", "skato_pvalue", "cmc_pvalue", "asum_pvalue", "acat.pvalue", "fisher.separate.pvalue")
load("../output/Var_specific_bargamma_MixedGene_MixtureVariant/7methods_delta0.01_gammamean335_3.RData")
AUC_summary_0.01=matrix(nrow=10, ncol=6)
colnames(AUC_summary_0.01)=c("ACAT", "ASUM", "Burden", "CMC", "MIRAGE", "SKAT-O")
#for (i in 1:10)
{
  i=10
Ui=Gene.Risk.Status[[i]]
...
method=rep(c("Burden(0.719)", "SKAT-O(0.759)", "MIRAGE(0.883)", "CMC(0.732)", "ASUM(0.703)", "ACAT(0.504)"), each=num_run) # AUC is from round(calc_auc(p1.basic)$AUC, 2) 
...
AUC_summary_0.01[i,]=calc_auc(p1.basic)$AUC
}
...
p.basic_burden=ggplot(roc_single_run_burden, aes(d = D, m = m, color=method), size=2) + 
  geom_roc(increasing=F, n.cuts=0, labels=F)+
  style_roc(theme = theme_grey)
p_delta0.01_burden=p.basic_burden+
  #annotate("text", x =rep(0.7, 8), y = c(0.02,0.08, 0.14, 0.2, 0.25, 0.3, 0.35, 0.4), 
  #         label = paste(c("ASUM:AUC", "Burden_adj:AUC", "Burden:AUC=", "Burden_comb:AUC", "CMC:AUC", "MIRAGE-BF:AUC=", "MIRAGE-LRT:AUC=", "SKATO:AUC="), round(calc_auc(p.basic)$AUC, 2)), size=2)+
  ggtitle(expression(paste(delta, "=0.01")))+
  theme_classic()+
...
grid.arrange(p1_delta0.02, p2_delta0.05, p_delta0.1, p_delta0.2, nrow=2) 
...
```{r, echo=F, message=F, warning=F}
#pdf("C:\\Shengtong\\Research\\rare-var\\RareVariant\\2023_0413_Figure_for_paper\\Fig-power_comparison_three-burden-gene.pdf")
grid.arrange(p_delta0.02_burden, p_delta0.05_burden, p_delta0.1_burden, p_delta0.2_burden, nrow=2) 
#dev.off()
...
```
* use threshold `MAC<10`  for variant collapsing for ACAT



```{r, echo=F, message=F, warning=F}
rm("Ui", "mirage_pvalue", "mirage_BF", "fisher_pvalue", "skato_pvalue", "cmc_pvalue", "asum_pvalue", "acat.pvalue", "fisher.separate.pvalue")
load("../output/Var_specific_bargamma_MixedGene_MixtureVariant/7methods_delta0.01_MAC_threshold5.gammamean335_5rep1.RData")
AUC_summary_0.01=matrix(nrow=10, ncol=6)
colnames(AUC_summary_0.01)=c("ACAT", "ASUM", "Burden", "CMC", "MIRAGE", "SKAT-O")
#for (i in 1:10)
{
...
method=rep(c("Burden(0.629)", "SKAT-O(0.786)", "MIRAGE(0.862)", "CMC(0.819)", "ASUM(0.556)", "ACAT(0.557)"), each=num_run) # AUC is from round(calc_auc(p1.basic)$AUC, 2) with variables order as  ACAT, ASUM, Burden, CMC, MIRAGE, SKAT-o
...
################################ delta=0.01
rm("Ui", "mirage_pvalue", "mirage_BF", "fisher_pvalue", "skato_pvalue", "cmc_pvalue", "asum_pvalue", "acat.pvalue", "fisher.separate.pvalue")
load("../output/Var_specific_bargamma_MixedGene_MixtureVariant/7methods_delta0.1_MAC_threshold5.gammamean335_5rep1.RData")
AUC_summary_0.05=matrix(nrow=5, ncol=6)
colnames(AUC_summary_0.05)=c("ACAT", "ASUM", "Burden", "CMC", "MIRAGE", "SKAT-O")
#for (i in 1:5)
{
...
method=rep(c( "Burden(0.678)", "SKAT-O(0.708)", "MIRAGE(0.837)", "CMC(0.722)", "ASUM(0.653)", "ACAT(0.632)"), each=num_run)
# pvalues from calc_auc(p2.basic)$AUC with variables order as  ACAT, ASUM, Burden, CMC, MIRAGE, SKAT-o
...
```
* use threshold `MAC<5`  for variant collapsing for ACAT 



```{r, echo=F, message=F, warning=F}
rm("Ui", "mirage_pvalue", "mirage_BF", "fisher_pvalue", "skato_pvalue", "cmc_pvalue", "asum_pvalue", "acat.pvalue", "fisher.separate.pvalue")
load("../output/Var_specific_bargamma_MixedGene_MixtureVariant/7methods_delta0.01_MAC_threshold0.gammamean335_5rep1.RData")
AUC_summary_0.01=matrix(nrow=10, ncol=6)
colnames(AUC_summary_0.01)=c("ACAT", "ASUM", "Burden", "CMC", "MIRAGE", "SKAT-O")
#for (i in 1:10)
{
...
method=rep(c("Burden(0.697)", "SKAT-O(0.787)", "MIRAGE(0.928)", "CMC(0.762)", "ASUM(0.596)", "ACAT(0.600)"), each=num_run) # AUC is from round(calc_auc(p1.basic)$AUC, 2) 
...
################################ delta=0.1
rm("Ui", "mirage_pvalue", "mirage_BF", "fisher_pvalue", "skato_pvalue", "cmc_pvalue", "asum_pvalue", "acat.pvalue", "fisher.separate.pvalue")
load("../output/Var_specific_bargamma_MixedGene_MixtureVariant/7methods_delta0.1_MAC_threshold0.gammamean335_5rep1.RData")
AUC_summary_0.05=matrix(nrow=5, ncol=6)
colnames(AUC_summary_0.05)=c("ACAT", "ASUM", "Burden", "CMC", "MIRAGE", "SKAT-O")
#for (i in 1:5)
{
...
method=rep(c( "Burden(0.692)", "SKAT-O(0.722)", "MIRAGE(0.856)", "CMC(0.720)", "ASUM(0.623)", "ACAT(0.586)"), each=num_run)
...
Simulated one data set, $N1=N0=1000, 3000, 5000$, 10\% of 1000 genes are risk genes. Every gene has 100 variants in three variant  groups with $\eta_1=0.05, \eta_2=0.2, \eta_3=0.5$ and $\bar{\gamma}=3,3,5$ respectively and the proportion of variants are 60\%, 30\% and 10\%. Other parameters are pretty the same as before 
$\alpha_0=0.1, \beta_0=1000,\alpha= 0.1, \beta=2000,\sigma= 1$.


```{r, echo=F, message=F, warning=F}
num.run=2; delta.index=4
bayes.fdr=seq(0, 0.5, length=100)
################## true gamma.bar=5, use true value  sample size 1000 ###############
rm("beta.k", "BF.gene", "mirage.pvalue", "skat.pvalue", "fisher.pvalue", "all.pi")
load("../output/BayesianFDR/Mixed_Gene_Gammamean5.delta0.1.SS1000.RData")
BF.gene=BF.gene[complete.cases(BF.gene), ]
Bayes_factor=tibble(BF=BF.gene[nrow(BF.gene),])

########## calculate post prob #############
prior_cons=0.1   # true value 
#prior_cons=all.pi[num.run, delta.index]
...
######### use 0.5 as post prob cutoff ########  
#(No.Disc=cons_post %>% filter(post_prob>0.5) %>% tally()) # use 0.5 as cutoff 

#(TP=cons_post %>% filter(post_prob>0.5) %>% filter(RiskStatus==1) %>% tally())

#FDR=1-TP/No.Disc
#FDR
####################  use different cutoffs for post prob #########
tau=seq(0, 0.999, length=1000)
num_pred=NULL
false_disc=NULL
FDR=NULL
...
actual.fdr=numeric()
tau.select=numeric()
for (i in 1:length(bayes.fdr))
{
  tau.select[i]=max(tau_fdr5 %>% filter(bar5_fdr>bayes.fdr[i] & bar5_fdr<bayes.fdr[i]+0.1) %>% pull(tau))
  No.Disc=cons_post %>% filter(post_prob>tau.select[i]) %>% tally() %>% pull()
  TP=cons_post %>% filter(post_prob>tau.select[i]) %>% filter(RiskStatus==1) %>% tally() %>% pull()
  # pull is used to convert tibble to a vector 
  actual.fdr[i]=1-TP/No.Disc
}
combine.fdr.SS1000=tibble(Bayes.FDR=bayes.fdr, Actual.FDR=actual.fdr) %>% add_column(SampleSize=1000)

################################################################################################################

################## true gamma.bar=5, use true value  sample size 3000 ###############
rm("beta.k", "BF.gene", "mirage.pvalue", "skat.pvalue", "fisher.pvalue", "all.pi")
load("../output/BayesianFDR/Mixed_Gene_Gammamean5.delta0.1.SS3000.2.RData")
BF.gene=BF.gene[complete.cases(BF.gene), ]
Bayes_factor=tibble(BF=BF.gene[nrow(BF.gene),])

########## calculate post prob #############
prior_cons=0.1  
#prior_cons=all.pi[1, delta.index]
cons_post=Bayes_factor %>% mutate(post_prob=prior_cons*BF/(1-prior_cons+prior_cons*BF))
cons_post=as_tibble(cbind(cons_post, RiskStatus=Ui))
...
combine.fdr.SS3000=tibble(Bayes.FDR=bayes.fdr, Actual.FDR=actual.fdr) %>% add_column(SampleSize=3000)



################## true gamma.bar=5, use true value  sample size 5000 ###############
rm("beta.k", "BF.gene", "mirage.pvalue", "skat.pvalue", "fisher.pvalue", "all.pi")
load("../output/BayesianFDR/Mixed_Gene_Gammamean5.delta0.1.SS5000.4.RData")
BF.gene=BF.gene[complete.cases(BF.gene), ]
Bayes_factor=tibble(BF=BF.gene[nrow(BF.gene),])
...
combine.fdr.SS5000=tibble(Bayes.FDR=bayes.fdr, Actual.FDR=actual.fdr) %>% add_column(SampleSize=5000)

combine.fdr=bind_rows(combine.fdr.SS1000, combine.fdr.SS3000, combine.fdr.SS5000)


#pdf("C:/Users/han24/OneDrive - UWM/rare-var/Figure/Supp/Fig-MIRAGE_gene_Bayes_FDR.pdf")
ggplot(combine.fdr, aes(x=Bayes.FDR, y=Actual.FDR))+
...
Simulated one data set, $N1=N0=3000$, 10\% of 1000 genes are risk genes. Every gene has 100 variants in three variant  groups with $\eta_1=0.05, \eta_2=0.2, \eta_3=0.5$ and the proportion of variants are 30\%, 40\% and 30\%. Each data set has one fixed $\bar{\gamma}$ and it is varied at $\bar{\gamma}=3,4,5,6$. Other parameters are pretty the same as before 
$\alpha_0=0.1, \beta_0=1000,\alpha= 0.1, \beta=2000,\sigma= 1$.
