...
summary=tibble(eta_est=all.pi[,1], MIRAGE.pvalue=unlist(MIRAGE.pvalue), Fisher.pvalue=unlist(Fisher.pvalue), SKATO.pvalue=unlist(SKATO.pvalue))
pvalue.cutoff=0.05
summary %>% filter(MIRAGE.pvalue<pvalue.cutoff) %>% tally()
summary %>% filter(Fisher.pvalue<pvalue.cutoff) %>% tally()
summary %>% filter(SKATO.pvalue<pvalue.cutoff) %>% tally()
